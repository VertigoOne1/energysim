...
class ExternalTaskExecutor:
    ...
    def execute_task(self, task, action):
        ...
        logger.trace(f"Executing external task for Topic: {topic} - {task_id}")
    ...
    def _handle_task_result(self, task_result):
        ...
        if task_result.is_success():
            logger.trace(f"Task result success - {task_result} - {topic} - {task_id}")
            self._handle_task_success(task_id, task_result, topic)
        elif task_result.is_bpmn_error():
            logger.trace(f"Task result error - {task_result} - {topic} - {task_id}")
            self._handle_task_bpmn_error(task_id, task_result, topic)
        elif task_result.is_failure():
            logger.trace(f"Task result failure - {task_result} - {topic} - {task_id}")
            ...
            logger.error(f"{err_msg} - {task_id}")
    ...
    def _handle_task_success(self, task_id, task_result, topic):
        logger.trace(f"Marking task complete for Topic: {topic} - {task_id}")
        ...
        if self.external_task_client.complete(task_id, task_result.global_variables, task_result.local_variables):
            logger.trace(f"Marked task completed - Topic: {topic} - {task_id}")
            ...
            logger.trace(f"Not able to mark task as complete - Topic: {topic} - {task_id}")
    ...
    def _handle_task_failure(self, task_id, task_result, topic):
        logger.trace(f"Marking task failed - Topic: {topic} task_result: {task_result} - {task_id}")
    ...
    def _handle_task_bpmn_error(self, task_id, task_result, topic):
        ...
        if bpmn_error_handled:
            logger.trace(f"BPMN Error Handled: {bpmn_error_handled} - Topic: {topic} - task_result: {task_result}")
            self._log_with_context(f"BPMN Error Handled: {bpmn_error_handled} "
                                   f"Topic: {topic} task_result: {task_result}")
        else:
            logger.trace(f"BPMN Error not Handled : {bpmn_error_handled} - Topic: {topic} - task_result: {task_result}")
