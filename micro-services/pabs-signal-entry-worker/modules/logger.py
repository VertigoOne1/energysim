...
KAFKA_DEBUG_LEVEL = str(config['kafka']['debug_level']).upper()
...
def setup_custom_logger(name):
    ...
    if config["kafka"]["enable_kafka_logging"]:
        kafkalevel = logging.getLevelName(KAFKA_DEBUG_LEVEL)
        kafkalogger = logging.getLogger('kafka')
        kafkalogger.addHandler(screen_handler)
        kafkalogger.setLevel(kafkalevel)
        kafkalogger.propagate = False

    if config["logging"]["enable_kafka_log_handler"]:
        kafka_handler = eventbus.KafkaLoggingHandler()
        kafka_handler.setFormatter(formatter)
        logger.addHandler(kafka_handler)
        logger.propagate = False
        logger.info("Kafka logging handler enabled")
    else:
        logger.info("Kafka logging handler disabled")

    telethonlogger = logging.getLogger('telethon')
    if (telethonlogger.hasHandlers()):
        logger.debug("Telethon - Clearing existing handlers")
        telethonlogger.handlers.clear()
    telethonlogger.addHandler(screen_handler)
    telethonlogger.setLevel(DEBUG_LEVEL)
    telethonlogger.propagate = False

    binance_wss_logger = logging.getLogger('unicorn_binance_websocket_api')
    if (binance_wss_logger.hasHandlers()):
        logger.debug("Unicorn - Clearing existing handlers")
        binance_wss_logger.handlers.clear()
    binance_wss_logger.addHandler(screen_handler)
    binance_wss_logger.setLevel(logging.getLevelName('WARN'))
    binance_wss_logger.propagate = False
